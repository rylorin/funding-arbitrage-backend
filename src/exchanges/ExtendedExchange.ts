...
export class ExtendedExchange extends ExchangeConnector {
  ...
  //   // Place the order using IOC (Immediate Or Cancel) time in force
  //   const placedOrder = await this.placeOrder(order, reduceOnly);

  //   // Extended uses IOC, so order should be filled or cancelled immediately
  //   // But we still poll to check for edge cases and consistency
