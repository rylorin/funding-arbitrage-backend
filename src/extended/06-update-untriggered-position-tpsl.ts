...
import { Decimal, type Long } from "./utils/number";
...
const runExample = async () => {
  ...
  const roundPrice = (value: Decimal) => {
    return roundToMinChange(value, market.tradingConfig.minPriceChange, Decimal.ROUND_DOWN);
  };

  const tpSlOrder = orders.find(
    (order) => order.type === "TPSL" && Boolean(order.tpSlType) && order.id.eq(TPSL_ORDER_ID),
  );
