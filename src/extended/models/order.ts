...
import { calcEntirePositionSize } from "../utils/calculation/calc-entire-position-size";
import { generateNonce } from "../utils/generate-nonce";
import { toHexString } from "../utils/hex";
import { Decimal, Long } from "../utils/number";
import { omitUndefined } from "../utils/omit-undefined";
import { getOppositeOrderSide } from "../utils/side";
import { getStarknetOrderMsgHash } from "../utils/signing/get-order-msg-hash";
import { getStarkPublicKey } from "../utils/signing/get-stark-public-key";
import { signMessage } from "../utils/signing/sign-message";
import { OrderDebuggingAmounts } from "./order-debugging-amounts";
...
export class Order {
  ...
  static create({
    ...
    const expiryEpochMillis = (expiryTime ?? addHours(new Date(), ORDER_EXPIRATION_HOURS)).getTime();
  ...
  private static getCreateOrderParams({
    ...
    const collateralAmountStark = collateralAmount.times(settlementResolutionCollateral).integerValue(roundingMode);
    const feeStark = fee.times(settlementResolutionCollateral).integerValue(ROUNDING_MODE_FEE);
    const syntheticAmountStark = amountOfSynthetic.times(settlementResolutionSynthetic).integerValue(roundingMode);
