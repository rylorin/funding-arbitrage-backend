...
app.get("/health", async (_req, res) => {
  try {
    const healthServiceStatus = healthService.getLastHealthStatus();
    if (process.env.NODE_ENV === "production") res.json(healthServiceStatus);
    else res.send(JSON.stringify(healthServiceStatus, null, 2));
  } catch (error) {
    res.status(500).json({
      status: "error",
