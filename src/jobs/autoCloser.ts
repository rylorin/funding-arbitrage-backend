  ...
  public async processAutoClosures(): Promise<JobResult> {
    ...
    try {
      ...
      for (const position of positionsToClose) {
        try {
          ...
          if (closureResult.success) {
            ...
            console.log(
              `✅ Auto-closed position ${position.id}: ${position.closedReason}`
            );
          } else {
            failedClosures.push({
              position,
          ...
          console.error(
            `❌ Failed to auto-close position ${position.id}:`,
            error
          );
        }
      }
      ...
      if (failedClosures.length === 0) {
        console.log(
          `✅ Auto closure completed: ${result.message} (${executionTime}ms)`
        );
      } else {
        console.log(
          `⚠️ Auto closure completed with failures: ${result.message} (${executionTime}ms)`
        );
      }
  ...
  private async closePosition(
    ...
    try {
      ...
      if (position.longPositionId && position.longExchange === "vest") {
        if (vestExchange.isConnected) {
          closurePromises.push(
            vestExchange.closePosition(position.longPositionId)
          );
        } else {
      ...
      if (position.shortPositionId && position.shortExchange === "vest") {
        if (vestExchange.isConnected) {
          closurePromises.push(
            vestExchange.closePosition(position.shortPositionId)
          );
        } else {
          throw new Error("Vest exchange not connected");
        }
      ...
      if (failures.length > 0) {
        throw new Error(
          `Failed to close ${failures.length} out of ${results.length} positions`
        );
      }
