...
import { Position, FundingRate, TradeHistory } from '../models/index';
...
interface PositionAlert {
  positionId: string;
  type: 'PROFIT_TARGET' | 'STOP_LOSS' | 'AUTO_CLOSE_TRIGGERED' | 'FUNDING_CHANGE' | 'PRICE_DEVIATION';
  message: string;
...
export class PositionMonitoringService {
  ...
  public async monitorPositions(): Promise<JobResult> {
    ...
    try {
      ...
      for (const position of activePositions) {
        try {
          ...
          if (shouldAutoClose.shouldClose) {
            const closeResult = await this.autoClosePosition(position, shouldAutoClose.reason);
            if (closeResult) {
              actionsPerformed.push(`Auto-closed position ${position.id}: ${shouldAutoClose.reason}`);
            }
          }
        } catch (error) {
      ...
      if (alerts.some(a => a.severity === 'CRITICAL')) {
        console.log(`⚠️ Position monitoring completed with CRITICAL alerts: ${result.message}`);
      } else {
  ...
  private async monitorSinglePosition(position: any): Promise<PositionAlert[]> {
    ...
    try {
      ...
      // Check profit target
      if (position.autoProfitTarget && currentPnL >= position.autoProfitTarget) {
        alerts.push({
          positionId: position.id,
          type: 'PROFIT_TARGET',
      ...
      if (priceDeviation > 1.0) { // More than 1% deviation
        alerts.push({
          positionId: position.id,
  ...
  private async checkAutoCloseConditions(position: any): Promise<{ shouldClose: boolean; reason: string }> {
    try {
      ...
      // Check PnL stop loss
      if (position.autoClosePnLThreshold && currentPnL <= -Math.abs(position.autoClosePnLThreshold)) {
        return { shouldClose: true, reason: `Stop loss hit: PnL ${currentPnL.toFixed(2)}` };
      }

      // Check APR threshold
      if (position.autoCloseAPRThreshold && currentAPR < position.autoCloseAPRThreshold) {
        return { shouldClose: true, reason: `APR below threshold: ${currentAPR.toFixed(2)}%` };
      }

      // Check timeout
      if (position.autoCloseTimeoutHours && hoursOpen >= position.autoCloseTimeoutHours) {
        return { shouldClose: true, reason: `Timeout reached: ${hoursOpen.toFixed(1)}h` };
      }
  ...
  private async autoClosePosition(position: any, reason: string): Promise<boolean> {
    try {
      ...
      console.log(`${longClosed && shortClosed ? '✅' : '⚠️'} Position ${position.id} auto-close ${longClosed && shortClosed ? 'completed' : 'partially failed'}`);
