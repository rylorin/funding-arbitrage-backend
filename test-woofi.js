...
class TestWoofiExchange {
  ...
  async getFundingRates(tokens) {
    try {
      ...
      console.log("📊 Fetching funding rates from Orderly (Orderly Network)...");
  ...
  async testExchangeInfo() {
    try {
      console.log("🧪 Testing Orderly exchange info endpoint...");
      ...
      const relevantMarkets =
        response.data.data?.rows?.filter((market) =>
      ...
      if (relevantMarkets.length > 0) {
        ...
        relevantMarkets.forEach((market) => {
          console.log(
            `  ${market.symbol}: funding_period=${market.funding_period}h, cap=${market.cap_funding}, floor=${market.floor_funding}`,
          );
        });
      }
      console.log("");
...
async function testWoofiConnector() {
  console.log("🧪 Testing Orderly (Orderly Network) Exchange Connector\n");

  const orderly = new TestWoofiExchange();
  const tokens = ["BTC", "ETH", "SOL"];

  try {
    // First test basic connectivity
    await orderly.testExchangeInfo();

    // Then test funding rates
    const rates = await orderly.getFundingRates(tokens);
